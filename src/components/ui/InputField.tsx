            ...
            <div className="w-full space-y-2 group">
                ...
                <div className="relative">
                    <input
                        id={generatedId}
                        ref={ref}
                        className={`flex h-[52px] w-full rounded-xl border-2 border-[#d4af37]/60 bg-white px-4 py-2 text-sm text-[#4a0404] placeholder:text-[#5d4037]/60 font-medium transition-all duration-300 focus:outline-none focus:ring-4 focus:ring-[#d4af37]/10 focus:border-[#d4af37] hover:border-[#d4af37] hover:shadow-md disabled:cursor-not-allowed disabled:opacity-50 shadow-sm ${error ? 'border-red-500 focus:ring-red-500/10 focus:border-red-500' : ''
                            } ${className}`}
                        {...props}
